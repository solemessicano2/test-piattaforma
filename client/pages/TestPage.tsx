...
export default function TestPage() {
  ...
  console.log(
    `Using test data with ${testData?.items?.length || 0} items:`,
    testData?.title,
  );
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span
                  className={`font-mono font-medium ${timeRemaining < 300 ? "text-red-600" : "text-gray-700"}`}
                >
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-gray-500">
                  di {testData?.items?.length || 0}
                </span>
              </div>
              <Badge
                variant="outline"
                className={`${
                  currentQ.domain === "Affettività Negativa"
                    ? "bg-red-50 text-red-700 border-red-200"
                    : currentQ.domain === "Distacco"
                      ? "bg-blue-50 text-blue-700 border-blue-200"
                      : currentQ.domain === "Antagonismo"
                        ? "bg-purple-50 text-purple-700 border-purple-200"
                        : currentQ.domain === "Disinibizione"
        ...
        <Card className="mb-8 border-0 shadow-xl bg-white/95 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            ...
            >
              {(testData.scaleLikert || testData.scaleOptions || []).map(
                (option, index) => {
                  const optionText =
            ...
            {answeredQuestions < (testData?.items?.length || 0) && (
              <div className="flex items-center space-x-2 text-amber-600">
                ...
                <span className="text-sm">
                  {(testData?.items?.length || 0) - answeredQuestions} domande
                  rimanenti
                </span>
              </div>
            )}
