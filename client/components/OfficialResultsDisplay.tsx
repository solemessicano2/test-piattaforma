...
export default function OfficialResultsDisplay({ profile, answers }: OfficialResultsDisplayProps) {
  const averageMeanScore = profile.domainScores.reduce((sum, d) => sum + d.meanScore, 0) / profile.domainScores.length;
  
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case "Molto Elevato": return "border-red-500 bg-red-50 text-red-800";
      case "Elevato": return "border-orange-500 bg-orange-50 text-orange-800";
      case "Moderatamente Elevato": return "border-yellow-500 bg-yellow-50 text-yellow-800";
      case "Medio": return "border-blue-500 bg-blue-50 text-blue-800";
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className={`border-2 shadow-xl ${getSeverityColor(profile.overallSeverity)}`}>
        <CardContent className="p-8">
          <div className="grid md:grid-cols-3 gap-8 items-center">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-500">(Scala 0-3 secondo DSM-5)</div>
            </div>
            <div className="space-y-4">
              <h3 className="text-2xl font-bold text-gray-900">
                Severità: {profile.overallSeverity}
              </h3>
              {profile.domainScores.filter(d => d.meanScore >= 2.0).length > 0 && (
                <div>
                  <p className="text-gray-600 mb-2">Domini Elevati (≥2.0):</p>
                  <div className="flex flex-wrap gap-2">
                    {profile.domainScores.filter(d => d.meanScore >= 2.0).map((domain) => (
                      <Badge key={domain.domain} className={getDomainColor(domain.domain)}>
            ...
            <div className="text-center">
              {profile.overallSeverity === "Basso" || profile.overallSeverity === "Molto Basso" ? (
            ...
            <div key={index} className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-lg font-bold">{domain.meanScore.toFixed(2)}</span>
                </div>
              </div>
              <Progress value={(domain.meanScore / 3) * 100} className="h-3" />
              <div className="text-sm text-gray-600">
                {domain.interpretation !== 'Non calcolabile (faccette principali incomplete)' ? (
                  <>
                    <strong>Faccette principali:</strong> {domain.facets.map(f => f.facet).join(', ')}
                    <br />
              ...
              {domain.meanScore >= 2.0 && (
                <Alert className="border-orange-200 bg-orange-50">
                  ...
                  <AlertDescription>
                    Punteggio clinicamente elevato (≥2.0) - Richiede attenzione clinica
                  </AlertDescription>
                </Alert>
              )}
            </div>
      ...
      <Card className="border-0 shadow-lg">
        ...
        <CardContent className="space-y-4">
          <Alert className="border-amber-200 bg-amber-50">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>
              <strong>Importante:</strong> Questi risultati utilizzano il sistema di scoring ufficiale del DSM-5 
              e non sostituiscono una valutazione clinica professionale.
            </AlertDescription>
          </Alert>
            ...
            {profile.facetScores.map((facet, index) => (
              <div key={index} className="p-3 border rounded-lg bg-gray-50">
                ...
                <div className="text-xs text-gray-600 mb-2">
                  Punteggio: {facet.meanScore.toFixed(2)} ({facet.interpretation})
                </div>
